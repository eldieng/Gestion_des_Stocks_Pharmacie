<?php
// Page de modification d'utilisateur (admin uniquement)
session_start();
...
if (!isset($_SESSION['user_id']) || !user_has_permission($pdo, $_SESSION['user_id'], 'modifier_utilisateur')) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $email_nouveau = trim($_POST['email'] ?? '');
    $role_id_nouveau = $_POST['role_id'] ?? '';
    $mot_de_passe = $_POST['mot_de_passe'] ?? '';
    $mot_de_passe2 = $_POST['mot_de_passe2'] ?? '';

    // Validation
    if (!$nom || !$prenom || !$email_nouveau || !$role_id_nouveau) {
        $message = "Tous les champs sont obligatoires (hors mot de passe).";
    } elseif (!filter_var($email_nouveau, FILTER_VALIDATE_EMAIL)) {
        $message = "Email invalide.";
    } elseif ($mot_de_passe && $mot_de_passe !== $mot_de_passe2) {
        $message = "Les mots de passe ne correspondent pas.";
    } else {
        // Vérifier unicité email si changé
        if ($email_nouveau !== $utilisateur['email']) {
            $stmt = $pdo->prepare('SELECT COUNT(*) FROM utilisateurs WHERE email = ? AND id != ?');
            $stmt->execute([$email_nouveau, $id]);
            if ($stmt->fetchColumn()) {
                $message = "Cet email existe déjà.";
            }
        }
        // Si pas d'erreur, update
        if (!$message) {
            $sql = 'UPDATE utilisateurs SET nom=?, prenom=?, email=?, role_id=?';
            $params = [$nom, $prenom, $email_nouveau, $role_id_nouveau];
            if ($mot_de_passe) {
                $sql .= ', mot_de_passe=?';
...
<head>
    ...
    <style>
        ...
        .modif-card { max-width: 430px; margin: 40px auto; border-radius: 1.2rem; }
        .modif-icon { font-size: 2.6rem; margin-bottom: 0.5rem; }
    </style>
</head>
<body>
    ...
    <div class="container">
        <div class="card shadow modif-card">
            <div class="card-body p-4">
                <div class="text-center mb-3">
                    <span class="modif-icon">📝</span>
