...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($nom && $quantite >= 0 && $prix_achat >= 0 && $prix_vente >= 0) {
        try {
            $stmt = $pdo->prepare('UPDATE medicaments SET nom=?, categorie=?, laboratoire=?, quantite=?, date_peremption=?, prix_achat=?, prix_vente=? WHERE id=?');
            $stmt->execute([$nom, $categorie, $laboratoire, $quantite, $date_peremption ?: null, $prix_achat, $prix_vente, $id]);
            $message = "Mise à jour effectuée avec succès !";
            // Rafraîchit les données
            $medicament = array_merge($medicament, [
                'nom' => $nom,
                'categorie' => $categorie,
...
<head>
    ...
    <title>Modifier un médicament - Fawsayni</title>
...
<body>
    <div class="container py-3">
        ...
        <div class="form-card">
            <h3 class="mb-3">Modifier un médicament</h3>
            ...
            <form method="post" action="">
                <div class="mb-3">
                    ...
                    <input type="text" name="nom" class="form-control" value="<?php echo htmlspecialchars($medicament['nom']); ?>" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Catégorie :</label>
                    <input type="text" name="categorie" class="form-control" value="<?php echo htmlspecialchars($medicament['categorie']); ?>">
                </div>
                <div class="mb-3">
                    <label class="form-label">Laboratoire :</label>
                    <input type="text" name="laboratoire" class="form-control" value="<?php echo htmlspecialchars($medicament['laboratoire']); ?>">
                </div>
                <div class="mb-3">
                    <label class="form-label">Quantité* :</label>
                    <input type="number" name="quantite" min="0" class="form-control" value="<?php echo (int)$medicament['quantite']; ?>" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Date de péremption :</label>
                    <input type="date" name="date_peremption" class="form-control" value="<?php echo htmlspecialchars($medicament['date_peremption']); ?>">
                </div>
                <div class="mb-3">
                    <label class="form-label">Prix d'achat* (FCFA):</label>
                    <input type="number" name="prix_achat" min="0" step="0.01" class="form-control" value="<?php echo htmlspecialchars($medicament['prix_achat']); ?>" required>
                </div>
                <div class="mb-3">
                    <label class="form-label">Prix de vente* (FCFA):</label>
                    <input type="number" name="prix_vente" min="0" step="0.01" class="form-control" value="<?php echo htmlspecialchars($medicament['prix_vente']); ?>" required>
                </div>
