<?php
// Page d'historique des achats (entrées de stock) avec filtres
...
$sql = 'SELECT a.id, a.quantite, a.date_achat, m.nom AS nom_medicament, u.nom AS nom_utilisateur, u.prenom AS prenom_utilisateur FROM achats a JOIN medicaments m ON a.id_medicament = m.id LEFT JOIN utilisateurs u ON a.utilisateur_id = u.id WHERE 1';
$params = [];
if ($medicament) {
    $sql .= ' AND m.nom LIKE ?';
    $params[] = "%$medicament%";
...
<head>
    <meta charset="UTF-8">
    <title>Historique des achats (entrées de stock)</title>
...
<body class="bg-light">
    <nav class="navbar navbar-expand-lg navbar-light bg-white shadow-sm mb-4">
        <div class="container">
            ...
            <div>
                <a href="enregistrer.php" class="btn btn-outline-primary btn-sm me-2">🛍️ Enregistrer un achat</a>
    ...
    <div class="container mb-5">
        ...
        <div class="card shadow-sm">
            <div class="card-body table-responsive p-0">
                <table class="table table-hover align-middle mb-0" id="achatsTable">
                    ...
                    <tbody>
                    <?php if (empty($achats)): ?>
                        <tr><td colspan="4" class="text-center text-muted py-4">Aucun achat enregistré.</td></tr>
                    <?php else: ?>
                        <?php foreach ($achats as $achat): ?>
                            <tr>
                                <td><span class="badge bg-info-subtle text-dark"><?php echo date('d/m/Y', strtotime($achat['date_achat'])); ?></span></td>
                                <td><?php echo htmlspecialchars($achat['nom_medicament']); ?></td>
                                <td>
                                    <span class="badge badge-quantite <?php
                                        $q = (int)$achat['quantite'];
                                ...
                                <td>
                                    <span class="avatar-initials">
                                        <?php
                                            $prenom = $achat['prenom_utilisateur'] ?? '';
                                            $nom = $achat['nom_utilisateur'] ?? '';
    ...
    let currentPage = 1;
    const tableBody = document.querySelector('#achatsTable tbody');
